...
//TODO for the future, it's possible if I cache the renderered models, modifying
// the passed model as I do in _parseSyntaxTree could cause issues.
//Clone?
    ...
    async.each(importModuleArray, (obj, callback) => {
      ...
      tools.resolveModulePath(editor.getPath(), source, (error, modulePath) => {
        ...
        var moduleModel = parseTreeModel(modulePath, (error, moduleModel) => {
          if (!error) {
            //We only want to see the exported symbols.
            if (moduleModel.children.length == 2 && moduleModel.children[0].name == "Imports") {
              //if this module is fairly complex it will have an imports an exports section
              let name = moduleModel.name;
