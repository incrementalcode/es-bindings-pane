...
export function parse(syntaxTree, rootName, callback) {
  try {
...
function _parse(syntaxTree, name = "root", loc = syntaxTree.loc, type = "Program", meta = null) {
  ...
  for (let _import of imports) {
    ...
    tools.resolveModulePath(editor.getPath(), node.source.value, (err, res) => {
      if (res) importModel.meta = path.normalize(res);
    });
